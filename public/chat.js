// public/chat.js - Truly Complete Version with All Functions & Height Fix (April 18, 2025)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function connectWebSocket() {
        ...
        socket.on('contactList', (contacts) => {
            console.log("[Contacts] Received contact list:", contacts); currentContacts = (contacts || []).map(c => ({...c, status: c.status || 'offline'})); if (!currentSearchTerm) { displayContactsOrSearchResults(currentContacts, false); }
        });
        ...
        socket.on('searchResultsUsers', (users) => {
            console.log("Received user search results:", users); if (currentSearchTerm) { displayContactsOrSearchResults(users, true); }
        });
        console.log("chat.js: Listener attached for 'searchResultsUsers'");

        socket.on('chatHistory', (data) => {
            console.log("chat.js: Event listener fired: chatHistory"); if (!data || !data.chatId || !Array.isArray(data.messages)) return; console.log(`[History] Received ${data.messages.length} messages for chat ${data.chatId}`); currentMessages[data.chatId] = data.messages; if (data.chatId === currentChatId) { if (messageList) messageList.innerHTML = ''; data.messages.forEach(displayMessage); scrollToBottom(); }
        });
        console.log("chat.js: Listener attached for 'chatHistory'");

        socket.on('receiveMessage', (message) => {
            console.log("chat.js: Event listener fired: receiveMessage"); if (!isSocketAuthenticated || !message || !message.sender) return; console.log('Message received:', message); const chatId = (message.sender === currentUser.id) ? message.recipientUid : message.sender; if (!chatId) return; if (!currentMessages[chatId]) currentMessages[chatId] = []; currentMessages[chatId].push(message); const otherUserId = (message.sender === currentUser.id) ? message.recipientUid : message.sender; const senderContactIndex = currentContacts.findIndex(c => c.id === otherUserId); if (senderContactIndex === -1) { console.log(`Adding sender ${otherUserId} to contact list state.`); currentContacts.push({ id: otherUserId, username: message.senderName || otherUserId, profilePicUrl: message.senderPic || null }); } updateContactPreview(chatId, message.content, message.timestamp, message.sender); if (chatId === currentChatId) { displayMessage(message); scrollToBottom(); } else { /* TODO: Increment unread count */ }
        });
    ...
    function displayContactsOrSearchResults(items, isSearchResult = false) {
        ...
        if (!isSearchResult) {
             itemsToDisplay.sort((a, b) => {
                  const statusA = a.status === 'online' ? 0 : 1; const statusB = b.status === 'online' ? 0 : 1; if (statusA !== statusB) return statusA - statusB; // Online first
                  // Use timestamp from contact object for sorting (this now comes from backend)
        ...
        itemsToDisplay.forEach(item => {
            ...
            // Use classList for dot visibility based on parent class
            ...
            if (isSearchResult) {
                ...
                // Check sender using lastMessageSenderUid field from backend
                ...
                let timestamp = item.timestamp || ''; // Use formatted timestamp from backend contact data
    ...
    function updateContactPreview(contactId, message, timestamp, senderId = null) {
        const contactIndex = currentContacts.findIndex(c => c.id === contactId); if (contactIndex === -1) return; const previewText = (senderId === currentUser.id) ? `You: ${message}` : message; currentContacts[contactIndex].lastMessage = previewText; currentContacts[contactIndex].timestamp = timestamp; currentContacts[contactIndex].lastMessageTimestamp = new Date(); currentContacts[contactIndex].lastMessageSenderUid = senderId; if (!currentSearchTerm) { displayContactsOrSearchResults(currentContacts, false); }
    }
    ...
    window.addEventListener('load', setAppHeight); // Adjust height on load
